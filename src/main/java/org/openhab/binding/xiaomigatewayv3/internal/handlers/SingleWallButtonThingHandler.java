...
/**
 * ThingHandler implementation for Aqara Single Wall Button (WXKG03LM)
 *
 * @author hubaksis - Initial contribution
 */
@NonNullByDefault
public class SingleWallButtonThingHandler extends AbstractXiaomiGatewayV3ThingHandler {

    //private final Logger logger = LoggerFactory.getLogger(AbstractXiaomiGatewayV3Handler.class);

    public SingleWallButtonThingHandler(Thing thing) {
        super(thing);

        LocalParamCollection.add(new SimpleEntry<String, String>("13.1.85", SINGLE_WALL_BUTTON_PRESSED));
    }    
